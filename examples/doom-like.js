...
var ASM_CONSTS = {
  82468: () => { Module.canvas.focus(); },  
 82493: () => { Module.useWebGL = true; GLImmediate.init(); },  
 ...
 83485: () => { var canvas = document.getElementById('canvas'); canvas.addEventListener('drop', function(e) { e.preventDefault(); if (e.dataTransfer.file < 0) return; var filenamesArray = []; var count = e.dataTransfer.files.length; var drop_dir = '.rgfw_dropped_files'; Module._RGFW_mkdir(drop_dir); for (var i = 0; i < count; i++) { var file = e.dataTransfer.files[i]; var path = '/' + drop_dir + '/' + file.name.replace("//", '_'); var reader = new FileReader(); reader.onloadend = (e) => { if (reader.readyState != 2) { out('failed to read dropped file: '+file.name+': '+reader.error); } else { var data = e.target.result; _RGFW_writeFile(path, new Uint8Array(data), file.size); } }; reader.readAsArrayBuffer(file); var filename = stringToNewUTF8(path); filenamesArray.push(filename); Module._RGFW_makeSetValue(i, filename); } Module._Emscripten_onDrop(count); for (var i = 0; i < count; ++i) { _free(filenamesArray[i]); } }, true); canvas.addEventListener('dragover', function(e) { e.preventDefault(); return false; }, true); },  
 84504: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 84575: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 84632: () => { return window.mouseX || 0; },  
 84663: () => { return window.mouseY || 0; },  
 84694: ($0) => { var canvas = document.getElementById('canvas'); if ($0) { canvas.style.pointerEvents = 'none'; } else { canvas.style.pointerEvents = 'auto'; } },  
 84841: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 84894: () => { return window.innerWidth; },  
 84924: () => { return window.innerHeight; },  
 84955: () => { Module.requestFullscreen(false, true); },  
 84994: () => { Module.exitFullscreen(false, true); },  
 85030: ($0, $1) => { var element = document.getElementById("canvas"); if (element) element.style.opacity = $1; }
};
